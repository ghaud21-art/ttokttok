...
function questionsPrompt(bookTitle, bookAuthor, context) {
  return `당신은 독서 성찰을 돕는 코치입니다. 아래는 "${bookTitle}"(${bookAuthor || '저자 미상'}) 책을 읽으며 독자가 기록한 인용구와 영감입니다.

${context || '(아직 기록된 인용구/영감이 없습니다. 책 제목을 바탕으로 질문을 만들어주세요.)'}

책의 메시지와 독자의 사고를 확장할 수 있는 한국어 질문 5개를 작성하세요.
1. 위 텍스트를 분석해 책의 주요 논점과 통찰을 도출하세요.
2. 다음 세 스타일을 내용에 따라 자유롭게 혼합해 질문을 작성하세요. 특정 장르로 억지로 유형화하지 마세요.
- 철학적 질문: 책의 논점을 깊이 성찰하도록 돕는 질문
- 실용적 질문: 독자가 책의 메시지를 자신의 삶에 적용하도록 돕는 질문
- 서사적 질문: 책의 내용과 감정적·주제적 연결을 탐구하도록 돕는 질문
3. 각 질문은 한 문장, 40자 내외로 간결하게 작성하세요.
...
function missionsPrompt(bookTitle, bookAuthor, context) {
  return `당신은 독서 후 실천을 설계하는 코치입니다. 아래는 "${bookTitle}"(${bookAuthor || '저자 미상'}) 책을 읽으며 독자가 남긴 인용구와 영감입니다.

${context || '(아직 기록된 인용구/영감이 없습니다. 책 제목을 바탕으로 미션을 만들어주세요.)'}

이 내용에서 책의 주제를 분석하고, 내가 책의 주제와 연관된 의미 있는 실천을 할 수 있도록 다음 조건을 충족하는 행동을 한국어로 5개 제안해주세요.
1. 주제와 긴밀히 연관되면서 책의 의도를 충분히 반영한 의미 있는 행동이어야 합니다. 단순히 텍스트를 변형하지 마세요.
2. 단순히 "적어보기"나 "찾아보기" 같은 활동 대신, 직접 체험하거나 도전할 수 있는 행동을 포함하세요.
3. 즉시 실행 가능하되, 독자가 직접 느끼고 사고를 확장하며 삶에 지속적인 영향을 줄 수 있는 행동을 제안하세요.
4. 매일/매주 반복해야 하는 습관성 목표는 제안하지 마세요.
5. 흥미롭고 창의적인 제안으로, 행동 자체가 독자에게 흥미와 동기를 부여할 수 있도록 하세요.
6. 각 액션은 50자 이내로 작성하세요.
