...
// value: 현재 공유 중인 group id 배열. onChange(nextArray)로 전체 목록을 넘겨준다.
    ...
    <div ref={ref} style={{ position: 'relative', flex: 'none' }} onClick={(e) => e.stopPropagation()}>
      ...
      {open && (
        <div
          ...
          style={{
            position: 'absolute', zIndex: 20, top: '110%', right: 0, minWidth: 160,
        ...
        >
          {groups.map((g) => (
            <label
              key={g.id}
              style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer', padding: '4px 6px', borderRadius: 6 }}
            >
